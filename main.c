...
typedef struct SyslogEntry {
    ...
    char facility[10];   // Kaynaði belirtir (Örnek: "auth", "kernel")
    ...
    struct SyslogEntry* next;  // Bir sonraki kaydi iþaret eden pointer
} SyslogEntry;
...
void addSyslogEntry(SyslogEntry** head, char timestamp[], char facility[], char severity[], char message[]) {
    // Yeni bir dügüm (node) için bellek ayýrma
    ...
    // Yeni dügümü listenin baþýna ekleme
...
void printSyslogEntries(SyslogEntry* head) {
    SyslogEntry* current = head;  // Listenin baþini iþaret eden geçici pointer
...
void freeSyslogEntries(SyslogEntry* head) {
    SyslogEntry* current = head;  // Listenin baþini iþaret eden gecici pointer
...
int main(int argc, char *argv[]) {
    ...
    // Bellegi temizleme (baðli listeyi silme)
